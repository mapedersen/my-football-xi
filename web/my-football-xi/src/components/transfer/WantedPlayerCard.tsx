...
import { removeWantedPlayer } from '@/services/transferService';
...
const WantedPlayerCard: React.FC<WantedPlayerCardProps> = ({ player, onUpdate }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [notes, setNotes] = useState(player.userNotes || "");
  
  const handleRemove = () => {
    removeWantedPlayer(player.id);
    onUpdate();
    toast.success(`${player.playerName} removed from wanted list`);
  };
  
  const handleSaveNotes = () => {
    // In a real app, this would update the player in the database
    player.userNotes = notes;
    setIsEditing(false);
