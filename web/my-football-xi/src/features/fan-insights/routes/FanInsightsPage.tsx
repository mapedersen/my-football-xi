...
const FanInsightsPage: React.FC = () => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="h-80">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Tooltip formatter={(value) => [`${value}%`]} />
                          <Legend />
                          <Line type="monotone" dataKey="sentiment" name="Fan Sentiment" stroke="#8884d8" activeDot={{ r: 8 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                    <p className="text-sm text-gray-500 text-center mt-4">
                      Fan sentiment timeline through the season
                    </p>
                  </CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="weakSpots" className="mt-4">
                <Card>
                  <CardContent className="pt-6">
                    <div className="h-80">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Tooltip formatter={(value) => [`${value}%`]} />
                          <Legend />
                          <Bar dataKey="percentage" name="Fan Agreement %" fill="#ff7675">
                            {identifiedWeakSpots.map((entry, index) => (
                              <Cell key={`cell-${index}`} fill={entry.percentage > 70 ? '#ff6b6b' : '#ffa94d'} />
                            ))}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    ...
                    <p className="text-sm text-gray-500 text-center mt-4">
                      Positions fans believe need strengthening
                    </p>
                  </CardContent>
                </Card>
              </TabsContent>
          ...
          <div className="lg:col-span-4 space-y-6">
                ...
                <div className="space-y-3">
                  {mostTrustedPlayers.map((player, index) => (
                    <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                      ...
                      <div className="text-green-600 font-semibold">{player.value}%</div>
                    </div>
                  ))}
                </div>
              </CardContent>
                ...
                <div className="space-y-3">
                  {mostDoubtedPlayers.map((player, index) => (
                    <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                      ...
                      <div className="text-red-600 font-semibold">{player.value}%</div>
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader className="pb-2">
                <h2 className="text-lg font-semibold flex items-center gap-2">
                  <Calendar className="w-5 h-5 text-united-red" />
                  Key Events
                </h2>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {sentimentTimeline.slice(0, 5).map((event, index) => (
                    <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                      <div className="flex-1">
                        <div className="text-sm font-medium">{event.event}</div>
                        <div className="text-xs text-gray-500">{event.date}</div>
                      </div>
                      <div className={`text-sm font-semibold ${event.sentiment > 65 ? 'text-green-600' : event.sentiment < 55 ? 'text-red-600' : 'text-yellow-600'}`}>
                        {event.sentiment}%
                      </div>
                    </div>
                  ))}
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-between items-center py-1 border-b border-gray-100">
                    <span className="text-sm text-gray-600">Active fans</span>
                    <span className="font-medium">12,489</span>
                  </div>
                  <div className="flex justify-between items-center py-1 border-b border-gray-100">
                    <span className="text-sm text-gray-600">Opinions submitted</span>
                    <span className="font-medium">47,325</span>
                  </div>
                  <div className="flex justify-between items-center py-1 border-b border-gray-100">
                    <span className="text-sm text-gray-600">Formations shared</span>
                    <span className="font-medium">8,216</span>
                  </div>
                  <div className="flex justify-between items-center py-1 border-b border-gray-100">
                    <span className="text-sm text-gray-600">Transfer rumors rated</span>
                    <span className="font-medium">15,782</span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="text-sm text-gray-600">Fan satisfaction</span>
...
export default FanInsightsPage;
